...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x, attn_mask=None):
        ...
        if self.flash:
            ...
            y = torch.nn.functional.scaled_dot_product_attention(q, k, v, attn_mask=attn_mask.to(torch.bool)[:, None, None, :] if attn_mask is not None else None, dropout_p=self.dropout if self.training else 0, is_causal=True)
        else:
...
class GPT(nn.Module):
    ...
    @torch.no_grad()
    def generate_masked(self, idx, max_new_tokens, temperature=1.0, top_k=None, attn_mask=None):
        """
